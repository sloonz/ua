...
class BMLyonBD(scrapy.Spider):
    ...
    def parse(self, response):
        for link in response.css("a.FacetLinkA::attr('href')").extract():
            if "bande_dessinees" in link:
                yield scrapy.Request(urlparse.urljoin(response.url, link), callback=self.parse_page)

    def parse_book(self, response, desc):
        body = response.css("td.DetailDataCell > table > tr > td > table").extract()[0]
        link = urlparse.urljoin(response.url, response.css("a[rel='bookmark']::attr('href')").extract()[0])
        img = response.css("img.LargeBookCover::attr('src')").extract()
        if img:
            img = urlparse.urljoin(response.url, img[0])
            body = u'<div><img src="%s"></div><div>%s</div>' % (img, body)
    ...
    def parse_page(self, response):
        def fetch_book(link, desc):
            return scrapy.Request(urlparse.urljoin(response.url, link),
        ...
        for item in response.css("#BrowseList a.SummaryFieldLink"):
            ...
            desc = u" ".join(item.css("::text").extract())
            yield fetch_book(link, desc)

        for item in response.css("a.pageNavLink::attr('href')").extract():
            if "PageDown" in item:
                yield scrapy.Request(urlparse.urljoin(response.url, item), callback=self.parse_page, dont_filter=True)
                break
